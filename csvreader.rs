                ...
                Payload::Some(dblock) => {
                    for series in dblock.data().iter() {
                        ...
                        rows.into_iter().for_each(|filename| {
                            let df1 = self.dataframe_from_filename(filename.unwrap());
                            let df2 = self.dataframe_from_filename(filename.unwrap());
                            let df3 = self.dataframe_from_filename(filename.unwrap());
                            let message1 = DataMessage::from(DataBlock::from(df1));
                            let message2 = DataMessage::from(DataBlock::from(df2));
                            let message3 = DataMessage::from(DataBlock::from(df3));
                            output_stream.write(message1);
                            output_stream.write(message2);
                            output_stream.write(message3);
                        });
                    }
                }
            }
